---
title: "Knowledge Base Index"
type: index
generated: 2026-04-23
---

# Knowledge Base Index

**15 articles** | Last generated: 2026-04-23

## Articles

- **[Agent Memory and Context Management](agent-memory-and-context-management.md)** — Memory is not an optional plugin for an agent harness -- it is the harness itself. How an agent manages what enters context, what gets evicted, what p...
- **[Agentic Design Patterns](agentic-design-patterns.md)** — Agent failures are more often **architectural** than prompting failures. An agent that loops endlessly lacks a stopping condition. An agent that calls...
- **[Auto Mode and Safety](auto-mode-and-safety.md)** — Claude Code's auto mode is a classification system that delegates permission approvals to model-based classifiers, providing a middle ground between m...
- **[Autoresearch and Self-Improvement](autoresearch-and-self-improvement.md)** — One of the most powerful applications of agent harnesses is turning agents on themselves -- using automated loops to iteratively improve agent skills,...
- **[Claude Code Architecture](claude-code-architecture.md)** — Claude Code is Anthropic's coding agent and one of the most extensively analyzed agent harnesses in production. After Anthropic accidentally shipped t...
- **[Deep Research Agents](deep-research-agents.md)** — Deep research agents represent a qualitative leap beyond single-turn LLM interactions. Rather than answering a question in one pass, these systems run...
- **[Lint Report 2026-04-13](lint-report-2026-04-13.md)** — - Total articles: 9
- Errors (must fix): 0
- Warnings (should fix): 7
- Info (nice to fix): 3
- **[Long-Running Agent Harnesses](long-running-agent-harnesses.md)** — Complex software projects cannot be completed within a single context window. Building agents that work effectively across multiple context windows --...
- **[Multi-Agent Reliability and Adversary Resistance](multi-agent-reliability.md)** — Multi-agent systems amplify the capabilities of individual LLM agents, but they also amplify failure modes. A single compromised, malfunctioning, or p...
- **[OpenAI Codex Harness](openai-codex-harness.md)** — OpenAI's Codex team published two foundational articles on harness engineering in early 2026, documenting their experience building a real product wit...
- **[Practical Best Practices](practical-best-practices.md)** — This article collects concrete, actionable practices for harness engineering -- the kind of hard-won lessons that emerge from building and shipping ag...
- **[Self-Evolving Agents and Skillify](self-evolving-agents.md)** — Self-evolving agents are systems that improve their own prompts, tools, memory, and workflows based on observed performance — without a human manually...
- **[Thin Harness, Fat Skills](thin-harness-fat-skills.md)** — "Thin harness, fat skills" is a three-tier agent architecture articulated by Garry Tan (YC President/CEO) in April 2026 after studying Anthropic's acc...
- **[Tool Design Patterns](tool-design-patterns.md)** — The tools available to an agent -- its action space -- are one of the most consequential design decisions in harness engineering. The accumulated evid...
- **[What Is Harness Engineering?](what-is-harness-engineering.md)** — Harness engineering is the discipline of configuring, extending, and structuring the integration points of an existing AI agent to maximize its effect...

## By Tag

### action-space
- [Tool Design Patterns](tool-design-patterns.md)

### adversary-resistance
- [Multi-Agent Reliability and Adversary Resistance](multi-agent-reliability.md)

### agent-architecture
- [Agent Memory and Context Management](agent-memory-and-context-management.md)
- [Claude Code Architecture](claude-code-architecture.md)
- [Long-Running Agent Harnesses](long-running-agent-harnesses.md)
- [Tool Design Patterns](tool-design-patterns.md)
- [What Is Harness Engineering?](what-is-harness-engineering.md)

### agent-first-development
- [OpenAI Codex Harness](openai-codex-harness.md)

### agent-protocol
- [Self-Evolving Agents and Skillify](self-evolving-agents.md)

### agentic-rag
- [Agentic Design Patterns](agentic-design-patterns.md)

### agents-md
- [OpenAI Codex Harness](openai-codex-harness.md)

### auto-mode
- [Auto Mode and Safety](auto-mode-and-safety.md)

### autocontext
- [Autoresearch and Self-Improvement](autoresearch-and-self-improvement.md)
- [Long-Running Agent Harnesses](long-running-agent-harnesses.md)

### autogenesis
- [Self-Evolving Agents and Skillify](self-evolving-agents.md)

### automated-optimization
- [Autoresearch and Self-Improvement](autoresearch-and-self-improvement.md)

### autoresearch
- [Autoresearch and Self-Improvement](autoresearch-and-self-improvement.md)

### benchmarks
- [What Is Harness Engineering?](what-is-harness-engineering.md)

### best-practices
- [Practical Best Practices](practical-best-practices.md)

### claude-code
- [Auto Mode and Safety](auto-mode-and-safety.md)
- [Claude Code Architecture](claude-code-architecture.md)

### codex
- [OpenAI Codex Harness](openai-codex-harness.md)

### codex-app-server
- [OpenAI Codex Harness](openai-codex-harness.md)

### context-efficiency
- [Tool Design Patterns](tool-design-patterns.md)

### context-engineering
- [Agent Memory and Context Management](agent-memory-and-context-management.md)
- [What Is Harness Engineering?](what-is-harness-engineering.md)

### context-management
- [Agent Memory and Context Management](agent-memory-and-context-management.md)
- [Long-Running Agent Harnesses](long-running-agent-harnesses.md)

### context-window
- [Agent Memory and Context Management](agent-memory-and-context-management.md)

### convergence
- [Deep Research Agents](deep-research-agents.md)

### credibility-scoring
- [Multi-Agent Reliability and Adversary Resistance](multi-agent-reliability.md)

### deep-research
- [Deep Research Agents](deep-research-agents.md)

### definitions
- [What Is Harness Engineering?](what-is-harness-engineering.md)

### design-patterns
- [Agentic Design Patterns](agentic-design-patterns.md)

### evaluation
- [Autoresearch and Self-Improvement](autoresearch-and-self-improvement.md)
- [Multi-Agent Reliability and Adversary Resistance](multi-agent-reliability.md)

### fat-code
- [Thin Harness, Fat Skills](thin-harness-fat-skills.md)

### fat-skills
- [Thin Harness, Fat Skills](thin-harness-fat-skills.md)

### filesystem
- [Agent Memory and Context Management](agent-memory-and-context-management.md)

### filesystem-abstraction
- [Tool Design Patterns](tool-design-patterns.md)

### git
- [Practical Best Practices](practical-best-practices.md)

### harness-design
- [Claude Code Architecture](claude-code-architecture.md)
- [Long-Running Agent Harnesses](long-running-agent-harnesses.md)

### harness-engineering
- [OpenAI Codex Harness](openai-codex-harness.md)
- [Practical Best Practices](practical-best-practices.md)
- [Thin Harness, Fat Skills](thin-harness-fat-skills.md)
- [What Is Harness Engineering?](what-is-harness-engineering.md)

### infrastructure
- [Tool Design Patterns](tool-design-patterns.md)

### iteration
- [Autoresearch and Self-Improvement](autoresearch-and-self-improvement.md)

### letta
- [Agent Memory and Context Management](agent-memory-and-context-management.md)

### lifecycle
- [Self-Evolving Agents and Skillify](self-evolving-agents.md)

### long-running-agents
- [Long-Running Agent Harnesses](long-running-agent-harnesses.md)

### mcp
- [Tool Design Patterns](tool-design-patterns.md)

### memgpt
- [Agent Memory and Context Management](agent-memory-and-context-management.md)

### memory
- [Agent Memory and Context Management](agent-memory-and-context-management.md)

### memory-ownership
- [Thin Harness, Fat Skills](thin-harness-fat-skills.md)

### model-generations
- [Practical Best Practices](practical-best-practices.md)

### multi-agent
- [Agentic Design Patterns](agentic-design-patterns.md)
- [Autoresearch and Self-Improvement](autoresearch-and-self-improvement.md)
- [Deep Research Agents](deep-research-agents.md)
- [Multi-Agent Reliability and Adversary Resistance](multi-agent-reliability.md)
- [Practical Best Practices](practical-best-practices.md)

### multi-context-window
- [Long-Running Agent Harnesses](long-running-agent-harnesses.md)

### openai
- [OpenAI Codex Harness](openai-codex-harness.md)

### orchestrator-worker
- [Deep Research Agents](deep-research-agents.md)

### permission-system
- [Claude Code Architecture](claude-code-architecture.md)

### permissions
- [Auto Mode and Safety](auto-mode-and-safety.md)

### planning
- [Agentic Design Patterns](agentic-design-patterns.md)

### progressive-disclosure
- [Claude Code Architecture](claude-code-architecture.md)
- [Practical Best Practices](practical-best-practices.md)

### prompt-engineering
- [Practical Best Practices](practical-best-practices.md)

### prompt-injection
- [Auto Mode and Safety](auto-mode-and-safety.md)

### react
- [Agentic Design Patterns](agentic-design-patterns.md)

### reflection
- [Agentic Design Patterns](agentic-design-patterns.md)

### reliability
- [Multi-Agent Reliability and Adversary Resistance](multi-agent-reliability.md)

### resolvers
- [Self-Evolving Agents and Skillify](self-evolving-agents.md)
- [Thin Harness, Fat Skills](thin-harness-fat-skills.md)

### rollback
- [Self-Evolving Agents and Skillify](self-evolving-agents.md)

### safety
- [Auto Mode and Safety](auto-mode-and-safety.md)

### search-reason-loops
- [Deep Research Agents](deep-research-agents.md)

### self-evolution
- [Self-Evolving Agents and Skillify](self-evolving-agents.md)

### self-improvement
- [Autoresearch and Self-Improvement](autoresearch-and-self-improvement.md)

### skill-refinement
- [Autoresearch and Self-Improvement](autoresearch-and-self-improvement.md)
- [Self-Evolving Agents and Skillify](self-evolving-agents.md)

### skillify
- [Self-Evolving Agents and Skillify](self-evolving-agents.md)
- [Thin Harness, Fat Skills](thin-harness-fat-skills.md)

### skills
- [Thin Harness, Fat Skills](thin-harness-fat-skills.md)

### sycophancy
- [Practical Best Practices](practical-best-practices.md)

### testing
- [Practical Best Practices](practical-best-practices.md)

### thin-harness
- [Thin Harness, Fat Skills](thin-harness-fat-skills.md)

### three-tier-architecture
- [Thin Harness, Fat Skills](thin-harness-fat-skills.md)

### tool-design
- [Claude Code Architecture](claude-code-architecture.md)
- [Tool Design Patterns](tool-design-patterns.md)

### tool-use
- [Agentic Design Patterns](agentic-design-patterns.md)

### tools
- [Tool Design Patterns](tool-design-patterns.md)

### transcript-classifier
- [Auto Mode and Safety](auto-mode-and-safety.md)

### uncategorized
- [Lint Report 2026-04-13](lint-report-2026-04-13.md)

### version-lineage
- [Self-Evolving Agents and Skillify](self-evolving-agents.md)

